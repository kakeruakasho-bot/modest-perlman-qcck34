...
const Card = ({ children, className = "" }) => (
  <div
    className={`bg-white rounded-2xl shadow-sm border border-stone-200 overflow-hidden ${className}`}
  >
    {children}
  </div>
);
...
const MonthNavigator = ({ currentMonth, onChange }) => (
  <div className="flex items-center justify-between bg-white p-3 rounded-2xl border border-stone-200 mb-6 shadow-sm print:hidden">
    <button
      onClick={() => onChange(-1)}
      className="p-3 text-stone-400 hover:bg-stone-100 hover:text-stone-600 rounded-xl active:scale-95 transition-all"
    >
      <ChevronLeft size={24} />
    </button>
    <div className="flex flex-col items-center">
      ...
      <span className="font-bold text-xl text-stone-800 tracking-tight">
        {String(currentMonth || "2026-01").split("-")[0]}年{" "}
        {String(currentMonth || "2026-01").split("-")[1]}月
      </span>
    </div>
    <button
      onClick={() => onChange(1)}
...
export default function App() {
  ...
  useEffect(() => {
    ...
    const unsubCategories = onSnapshot(
      query(collection(db, "artifacts", appId, "public", "data", "categories")),
      (snapshot) => {
    ...
    const unsubMenu = onSnapshot(
      query(collection(db, "artifacts", appId, "public", "data", "menu_items")),
      (snapshot) => {
    ...
    const unsubOrders = onSnapshot(
      query(collection(db, "artifacts", appId, "public", "data", "orders")),
      (snapshot) => {
        const data = snapshot.docs.map((doc) => ({
          id: doc.id,
          ...doc.data(),
    ...
    const unsubExpenses = onSnapshot(
      query(collection(db, "artifacts", appId, "public", "data", "expenses")),
      (snapshot) => {
        const data = snapshot.docs.map((doc) => ({
          id: doc.id,
          ...doc.data(),
    ...
    const unsubReports = onSnapshot(
      query(collection(db, "artifacts", appId, "public", "data", "reports")),
      (snapshot) => {
        const data = snapshot.docs.map((doc) => ({
          id: doc.id,
          ...doc.data(),
    ...
    const unsubFunds = onSnapshot(
      query(collection(db, "artifacts", appId, "public", "data", "funds")),
      (snapshot) => {
        const data = snapshot.docs.map((doc) => ({
          id: doc.id,
          ...doc.data(),
  ...
  const handlePrint = () => {
    ...
    if (isStandalone && isMobile) {
      alert(
        "【スマホのアプリモードでは印刷ができません】\n\nお手数ですが、印刷する時だけは「Safari」や「Chrome」などのブラウザアプリで直接このページ（URL）を開き直して、このボタンを押すかブラウザのメニューから印刷してください！🙇‍♂️"
      );
      return;
    }
    setTimeout(() => {
  ...
  const handleCheckout = async (paymentMethod = "cash") => {
    ...
    try {
      ...
      if (paymentMethod === "paypay") {
        await addDoc(
          collection(db, "artifacts", appId, "public", "data", "expenses"),
          {
            date: orderDate,
            item: "PayPay決済手数料",
            amount: Math.floor(totalAmount * 0.0198),
            payer: "ランタナ",
            category: "その他",
            createdAt: serverTimestamp(),
          }
      ...
      alert(
        `${paymentMethod === "paypay" ? "PayPay" : "現金"}でお会計しました！`
      );
    } catch (error) {
  ...
  const submitReport = async (e) => {
    ...
    try {
      await addDoc(
        ...
        {
          ...reportForm,
          customerCount: Number(reportForm.customerCount || 0),
          createdAt: serverTimestamp(),
        }
      );
  ...
  const handleAdjustment = async (e) => {
    ...
    const targetTax = Number(adjustTax || 0);
    const targetFund = Number(adjustFund || 0);

    const currentTax = aggregated.totalTaxFund || 0;
    const currentFund = (aggregated.fundBalance || 0) - currentTax;

    const diffTax = targetTax - currentTax;
    const diffFund = targetFund - currentFund;

    if (diffTax === 0 && diffFund === 0) {
      alert("現在の残高と変更がありません。");
      return;
    }
    ...
    if (
      !window.confirm(
        `以下の内容で残高を強制調整しますか？\n\n消費税の調整: ${
          diffTax >= 0
            ? "+" + diffTax.toLocaleString()
            : diffTax.toLocaleString()
        }円\nお店の資金の調整: ${
          diffFund >= 0
            ? "+" + diffFund.toLocaleString()
            : diffFund.toLocaleString()
        }円`
      )
    )
  ...
  const getAggregatedData = () => {
    ...
    const targetOrders = safeOrders.filter((o) =>
      String(o.date || "").startsWith(currentMonth)
    );
    const targetExpenses = safeExpenses.filter((e) =>
    ...
    targetOrders.forEach((order) => {
      ...
      if (order.paymentMethod === "paypay")
        paypaySales += Number(order.total || 0);
      else cashSales += Number(order.total || 0);
      if (order.items && Array.isArray(order.items)) {
        order.items.forEach((item) => {
          if (item.isTakeout) tax8Sales += Number(item.price || 0);
          else tax10Sales += Number(item.price || 0);
        });
      }
    });
    ...
    targetExpenses.forEach((exp) => {
      ...
      if (String(exp.item || "").includes("PayPay決済手数料"))
        paypayFee += Number(exp.amount || 0);
      ...
      if (exp.category !== "給料分配" && exp.category !== "雑収入・返金") {
        ...
        else if (exp.payer === "ランタナ")
          totalLantanaPay += Number(exp.amount || 0);
      }
    });
    ...
    const totalLantanaExpenses = safeExpenses
      .filter((e) => e.payer === "ランタナ")
      .reduce((sum, e) => sum + Number(e.amount || 0), 0);
    const totalTaxFund = safeFunds
      .filter((f) => {
    ...
    return {
      daily: Object.values(dataByDate).sort((a, b) =>
        String(b.date || "").localeCompare(String(a.date || ""))
      ),
      summary: {
        totalSales,
  ...
  const aggregated = useMemo(
    () => getAggregatedData(),
    [orders, expenses, funds, currentMonth]
  );

  const advancedAnalytics = useMemo(() => {
    let totalCustomers = 0;
    let totalSalesForSpend = 0;
    const weatherStats = {
      晴れ: { sales: 0, days: 0 },
      曇り: { sales: 0, days: 0 },
      雨: { sales: 0, days: 0 },
    ...
    (reports || []).forEach((r) => {
      if (r.date && String(r.date).startsWith(currentMonth)) {
        const daySales =
          aggregated.daily.find((d) => d.date === r.date)?.sales || 0;
        const w = r.weather || "晴れ";
        if (weatherStats[w] !== undefined) {
          weatherStats[w].sales += daySales;
          weatherStats[w].days += 1;
        }
        if (Number(r.customerCount || 0) > 0) {
          totalCustomers += Number(r.customerCount);
          totalSalesForSpend += daySales;
        }
      }
    });
    const avgSpend =
      totalCustomers > 0 ? Math.floor(totalSalesForSpend / totalCustomers) : 0;
    ...
    (expenses || []).forEach((e) => {
      if (
        ...
        expCategories[cat] = (expCategories[cat] || 0) + Number(e.amount);
      }
    });
    const sortedExpenses = Object.entries(expCategories).sort(
      (a, b) => b[1] - a[1]
    );
    const totalExpForChart = sortedExpenses.reduce(
      (sum, [, amt]) => sum + amt,
      0
    );
    const foodCost = expCategories["仕入"] || 0;
    const salesTotal = aggregated.summary.totalSales || 0;
    const costRatio =
      salesTotal > 0 ? ((foodCost / salesTotal) * 100).toFixed(1) : 0;

    let singleCount = 0;
    let setCount = 0;
    (orders || []).forEach((o) => {
      if (o.date && String(o.date).startsWith(currentMonth)) {
        (o.items || []).forEach((item) => {
          if (item.setType === "single") singleCount++;
          else setCount++;
        });
      }
    });
    const totalItems = singleCount + setCount;
    const setRate =
      totalItems > 0 ? Math.floor((setCount / totalItems) * 100) : 0;
  ...
  }, [orders, reports, expenses, aggregated, currentMonth]);
  ...
  const chartData = useMemo(() => {
    ...
    if (analysisPeriod === "week") {
      ...
      for (let i = 11; i >= 0; i--) {
        ...
        const monthlySales = safeOrders
          .filter((o) => String(o.date || "").startsWith(monthStr))
          .reduce((s, o) => s + Number(o.total || 0), 0);
        data.push({
      ...
      <div className="max-w-2xl mx-auto space-y-6 pb-20 animate-in fade-in duration-300 print:w-full print:max-w-none print:pb-0 print:space-y-4">
        <div className="flex justify-between items-center print:hidden">
          <h2 className="text-2xl font-bold text-stone-800 flex items-center gap-2">
            <History className="text-orange-600" /> 帳簿・決算
          </h2>
          <div className="flex gap-4 items-center">
            ...
            <div className="flex gap-2">
              <button
                onClick={handlePrint}
                className="text-sm bg-white border border-stone-200 text-stone-600 px-3 py-2 rounded-xl flex items-center gap-1 shadow-sm hover:bg-stone-50 transition-colors"
              >
                <Printer size={16} /> 印刷
              </button>
              <button
        ...
        <div className="hidden print:block text-center mb-6">
          <h1 className="text-2xl font-bold text-black border-b-2 border-black pb-2 inline-block">
            {String(currentMonth || "2026-01").split("-")[0]}年{" "}
            {String(currentMonth || "2026-01").split("-")[1]}月 決算帳簿
          </h1>
        </div>
        ...
        <div className="grid grid-cols-2 gap-4 print:gap-6">
          <Card className="p-5 border-l-4 border-l-blue-500 hover:shadow-md transition-shadow print:border-black print:border-l-2 print:shadow-none print:rounded-none">
            <p className="text-xs text-stone-500 font-bold mb-1 flex items-center gap-1 print:text-black">
              <ArrowDownCircle size={14} className="print:hidden" /> 売上合計
              (税込)
            </p>
            <p className="text-2xl font-mono font-bold text-stone-800 print:text-black">
            ...
            <div className="flex gap-2 mt-2 pt-2 border-t border-stone-100 text-[10px] text-stone-400 print:text-black print:border-black">
              <span>
                現金: ¥{aggregated.summary.cashSales.toLocaleString()}
              </span>
              <span>
                PayPay: ¥{aggregated.summary.paypaySales.toLocaleString()}
              </span>
            </div>
          </Card>
          <Card className="p-5 border-l-4 border-l-red-500 hover:shadow-md transition-shadow print:border-black print:border-l-2 print:shadow-none print:rounded-none">
            <p className="text-xs text-stone-500 font-bold mb-1 flex items-center gap-1 print:text-black">
              <ArrowRightCircle size={14} className="print:hidden" /> 経費合計
              (給料以外)
            </p>
            <p className="text-2xl font-mono font-bold text-stone-800 print:text-black">
            ...
            <div className="mt-3 pt-3 border-t border-stone-100 space-y-1.5 text-xs text-stone-600 print:text-black print:border-black">
              <div className="flex justify-between items-center">
                <span className="flex items-center gap-1">
                  <span className="w-1.5 h-1.5 rounded-full bg-orange-400 print:hidden"></span>
                  高橋 経費 (立替)
                </span>
              ...
              <div className="flex justify-between items-center">
                <span className="flex items-center gap-1">
                  <span className="w-1.5 h-1.5 rounded-full bg-green-400 print:hidden"></span>
                  ランタナ 経費 (お店払)
                </span>
                <span className="font-mono">
            ...
            {aggregated.summary.paypayFee > 0 && (
              <div className="mt-2 text-[10px] text-red-400 print:text-black">
                ※ランタナ経費のうち PayPay手数料: ¥
                {aggregated.summary.paypayFee.toLocaleString()}
              </div>
            )}
          </Card>
        </div>
        ...
        <Card className="bg-white border-2 border-orange-200 overflow-visible mt-8 relative print:border-none print:shadow-none print:mt-4 print:rounded-none">
          ...
          <div className="p-6 pt-8 print:p-0">
            <div className="flex justify-between items-center mb-2">
              <span className="font-bold text-stone-600 text-sm print:text-black">
                お店の粗利益
              </span>
              <span className="text-2xl font-mono font-bold text-stone-800 print:text-black">
                ¥{(aggregated.summary.profitBeforeTax || 0).toLocaleString()}
              </span>
            </div>
            ...
            <div className="bg-stone-50 rounded-xl p-5 border border-stone-200 print:bg-transparent print:border-none print:rounded-none print:p-0">
              <div className="flex justify-between items-center mb-6 pb-4 border-b border-stone-200 print:border-black print:mb-2 print:pb-2">
                ...
                <span className="text-2xl font-mono font-bold text-orange-600 print:text-black">
                  ¥{aggregated.summary.distributableProfit.toLocaleString()}
                </span>
              </div>
              ...
              {aggregated.summary.isClosed ? (
                <div className="text-center py-4 print:py-0">
                  <CheckCircle2
                    size={48}
                    className="mx-auto text-green-500 mb-3 print:hidden"
                  />
                  <p className="font-bold text-stone-800 mb-6 text-lg print:hidden">
                    今月の締め作業は完了しています！
                  </p>
                  <div className="bg-white p-4 rounded-lg shadow-sm border border-stone-100 space-y-3 text-left print:border-none print:shadow-none print:p-0">
                    <div className="flex justify-between items-center">
                      <span className="text-sm font-bold text-stone-600 print:text-black">
                        お店の資金へ追加した額:
                      </span>
                      <span className="font-mono font-bold text-lg text-green-600 print:text-black">
                        ¥{aggregated.summary.recordedFund.toLocaleString()}
                      </span>
                    </div>
                    <div className="flex justify-between items-center pt-3 border-t border-stone-100 print:border-black">
                      <span className="text-sm font-bold text-stone-600 print:text-black">
                        配った給料 (1人あたり):
                      </span>
                      <span className="font-mono font-bold text-lg text-blue-600 print:text-black">
                        ¥{aggregated.summary.recordedSalary.toLocaleString()}
                      </span>
                    </div>
                  </div>
                </div>
                ...
                <div className="space-y-8 print:hidden">
                  <div>
                    <label className="flex justify-between items-end mb-3">
                      ...
                      <div className="flex items-center gap-1">
                        <input
                          type="number"
                          value={lantanaFundInput}
                          onChange={(e) =>
                            setLantanaFundInput(Number(e.target.value))
                          }
                          className="w-28 p-2 text-right border border-orange-300 rounded-lg font-mono font-bold text-lg focus:outline-none focus:ring-2 focus:ring-orange-500"
                        />
                  ...
                  <div className="bg-white p-4 rounded-xl border-l-4 border-l-blue-500 shadow-sm">
                    ...
                    <p className="text-[10px] text-stone-400 text-right">
                      ※残額 {maxFund - currentFund} 円を2等分 (端数切捨て)
                    </p>
                  </div>
                  ...
                  <Button
                    onClick={() =>
                      executeMonthlyClosing(
                        aggregated.summary.totalTax,
                        currentFund,
                        calculatedSalary
                      )
                    }
                    className="w-full py-4 text-base shadow-lg shadow-orange-200"
                  >
        ...
        <div className="bg-white rounded-2xl border border-stone-200 shadow-sm overflow-hidden mt-8 print:border-black print:shadow-none print:rounded-none print:mt-6">
          <div className="bg-stone-50 p-3 border-b border-stone-200 font-bold text-stone-600 text-sm print:bg-transparent print:border-black print:text-black">
            日別の売上・経費一覧
          </div>
            ...
            <table className="w-full text-sm text-left">
              <thead className="bg-white text-stone-400 border-b border-stone-100 print:text-black print:border-black">
                <tr>
                  ...
                  <th className="p-4 text-right font-medium">売上</th>
                  <th className="p-4 text-right font-medium">経費</th>
                </tr>
              </thead>
                ...
                {aggregated.daily.map((row) => (
                  <React.Fragment key={row.date}>
                    <tr
                      ...
                      className="cursor-pointer hover:bg-orange-50 transition-colors print:hover:bg-transparent"
                    >
                      <td className="p-4 font-mono print:font-bold">
                        {String(row.date || "").slice(8, 10)}日
                      </td>
                      <td className="p-4 text-right font-mono text-stone-700 print:text-black print:font-bold">
                        ¥{(row.sales || 0).toLocaleString()}
                      </td>
                      <td className="p-4 text-right font-mono text-stone-500 print:text-black print:font-bold">
                    ...
                    <tr
                      className={`${
                        expandedDate === row.date ? "table-row" : "hidden"
                      } ${
                        printDetails ? "print:table-row" : "print:hidden"
                      } bg-stone-50 print:bg-transparent`}
                    >
                      <td
                        colSpan={3}
                        className="p-4 px-6 border-l-4 border-orange-300 print:border-none print:p-2 print:border-b print:border-stone-200"
                      >
                        <div className="space-y-3 print:space-y-1">
                          {row.expenseDetails.map((e) => (
                            <div
                              key={e.id || Math.random()}
                              className="flex justify-between text-xs items-center p-2 bg-white rounded-lg border border-stone-100 print:border-none print:p-0"
                            >
                              <span className="flex-1 text-stone-600 print:text-black">
                                <span
                                  className={`px-2 py-0.5 rounded text-[10px] font-bold mr-2 print:border print:border-black print:bg-transparent print:text-black ${
                                    e.payer === "ランタナ"
                              ...
                              <span className="font-mono print:text-black">
                                ...
                                <button
                                  ...
                                  className="ml-2 text-stone-300 hover:text-red-500 print:hidden"
                                >
                                  <Trash2 size={14} />
                                </button>
                              </span>
                          ...
                          {row.rawOrders.map((o) => (
                            <div
                              ...
                              className="p-2 bg-white rounded-lg border border-stone-100 print:border-none print:p-0"
                            >
                              <div className="flex justify-between text-xs font-bold text-stone-700 mb-1 print:text-black">
                                <span className="flex items-center gap-1">
                                  ...
                                  {o.paymentMethod === "paypay" && (
                                    <span className="text-[10px] bg-red-100 text-red-600 px-1.5 py-0.5 rounded print:border print:border-black print:bg-transparent print:text-black">
                                      PayPay
                                    </span>
                                  )}
                                </span>
                          ...
                          {row.expenseDetails.length === 0 &&
                            row.rawOrders.length === 0 && (
                              <div className="text-xs text-stone-400 text-center py-2 print:text-left print:py-0">
                                記録がありません
                              </div>
                            )}
                        </div>
  ...
  const renderFunds = () => (
    <div className="max-w-2xl mx-auto space-y-6 pb-20 animate-in fade-in duration-300">
      ...
      <div className="flex justify-end mb-4 z-10 relative">
        ...
        >
          <Settings size={12} />
          実際の口座残高とズレている場合はこちら（残高調整）
        </button>
      </div>

      <div className="bg-gradient-to-br from-stone-800 to-stone-900 rounded-3xl p-8 text-center shadow-xl relative overflow-hidden mt-0">
        <div className="absolute top-0 right-0 p-4 opacity-10">
          <Landmark size={120} />
        </div>
        <h3 className="text-stone-300 font-bold mb-2 flex items-center justify-center gap-2 text-sm z-10 relative">
          ランタナ預かり金 (お店の全資金)
        </h3>
        <div className="text-5xl font-mono font-bold text-white mb-6 tracking-tight z-10 relative">
          ¥{(aggregated.fundBalance || 0).toLocaleString()}
        </div>

        <div className="grid grid-cols-2 gap-4 text-left z-10 relative">
          <div className="bg-white/10 rounded-xl p-4 backdrop-blur-sm border border-white/10">
            <div className="text-stone-400 text-[10px] font-bold mb-1">
              お預かり消費税 (累計)
            </div>
      ...
      <Card className="p-6 border-none shadow-sm">
        <h2 className="text-lg font-bold text-stone-700 mb-6 flex items-center gap-2">
          <RefreshCw className="text-orange-600" /> 手動での資金移動記録
        </h2>
        <form onSubmit={submitFund} className="space-y-4">
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <input
                ...
                className="w-full p-3 border border-stone-200 rounded-xl bg-stone-50 focus:bg-white transition-colors outline-none focus:ring-2 focus:ring-orange-500"
              />
            </div>
            <div>
              <label className="block text-xs font-bold text-stone-500 mb-1">
          ...
          <div>
            ...
            <input
              ...
              className="w-full p-3 border border-stone-200 rounded-xl bg-stone-50 focus:bg-white transition-colors outline-none focus:ring-2 focus:ring-orange-500 font-mono text-lg"
            />
          </div>
          <div>
            <label className="block text-xs font-bold text-stone-500 mb-1">
      ...
      <div className="space-y-3">
        ...
        {funds.map((f) => (
          <div
            ...
            className="flex justify-between p-4 bg-white border border-stone-200 rounded-2xl items-center hover:shadow-md transition-shadow"
          >
            <div>
              <span className="text-[10px] text-stone-400 block mb-1 font-mono">
                {f.date || ""}
              </span>
            ...
            <div className="flex items-center gap-3">
              ...
              >
                {Number(f.amount || 0) >= 0 ? "+" : ""}¥
                {Math.abs(Number(f.amount || 0)).toLocaleString()}
              </span>
      ...
      {editingFund && (
        <div className="fixed inset-0 bg-black/50 flex items-center justify-center p-4 z-50 animate-in fade-in duration-200">
          <div className="bg-white p-6 rounded-3xl w-full max-w-sm shadow-2xl">
            <h3 className="font-bold mb-6 text-stone-800 text-lg">
              履歴を修正
            </h3>
            <form onSubmit={updateFund} className="space-y-4">
              <input
                ...
                className="border border-stone-200 p-3 w-full rounded-xl"
              />
              <select
                value={editingFund.type || "入金"}
                onChange={(e) =>
              ...
              <input
                ...
                className="border border-stone-200 p-3 w-full rounded-xl font-mono"
              />
              <input
                type="text"
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4 animate-in fade-in zoom-in-95 duration-200 print:hidden">
          <div className="bg-white rounded-3xl w-full max-w-md p-8 text-center space-y-6 shadow-2xl">
            ...
            <div>
              ...
              <p className="text-[10px] text-stone-500 mt-2 leading-relaxed text-left">
                実際の口座などにある「正しい金額」を入力してください。
                <br />
                現在の計算との差額を自動計算し、ピッタリ合うように調整データを記録します。
              </p>
            </div>
            ...
            <form onSubmit={handleAdjustment} className="space-y-4 text-left">
              <div>
                ...
                <input
                  ...
                  required
                  value={adjustTax}
                  onChange={(e) => setAdjustTax(e.target.value)}
              ...
              <div>
                ...
                <input
                  ...
                  required
                  value={adjustFund}
                  onChange={(e) => setAdjustFund(e.target.value)}
                  className="w-full p-3 border border-stone-200 rounded-xl bg-stone-50 outline-none focus:ring-2 focus:ring-orange-500 font-mono text-lg"
                  placeholder="例: 137209"
                />
              </div>
              ...
              <div className="bg-stone-50 p-4 rounded-xl border border-stone-100 mt-4">
                ...
                <p className="text-2xl font-mono font-bold text-stone-800">
                  ¥
                  {(
                    Number(adjustTax || 0) + Number(adjustFund || 0)
                  ).toLocaleString()}
                </p>
              </div>
              ...
              <div className="flex gap-3 pt-4">
                <Button
                  variant="secondary"
                  className="flex-1 py-4 border-none bg-stone-100"
                ...
                <Button
                  type="submit"
                  className="flex-1 py-4 shadow-lg shadow-orange-200"
                >
                  この金額に合わせる
                </Button>
              </div>
    ...
    <div className="max-w-2xl mx-auto space-y-6 pb-20 animate-in fade-in duration-300">
      <Card className="p-6">
        <h2 className="text-xl font-bold text-stone-700 mb-6 flex items-center gap-2">
          <DollarSign className="text-orange-600" /> 経費・収入の入力
        </h2>
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              ...
              <input
                ...
                className="w-full p-3 border border-stone-200 rounded-xl bg-stone-50 outline-none focus:ring-2 focus:ring-orange-500 font-mono text-lg text-right"
              />
            </div>
          </div>
          <div>
      ...
      <div className="space-y-3">
        ...
        {expenses.slice(0, 8).map((exp) => (
          <div
            ...
            className="bg-white p-4 rounded-2xl border border-stone-200 flex justify-between items-center hover:shadow-md transition-shadow"
          >
            <div>
              <div className="font-bold text-stone-700 text-sm mb-1">
                {exp.item || exp.category || "名称未設定"}
              </div>
              <div className="text-[10px] text-stone-400 font-mono">
                {exp.date || ""} / {exp.payer}
                {Number(exp.amount || 0) < 0 ? "受取" : "払"}
              </div>
            </div>
            <div
              className={`font-mono text-lg font-bold ${
                Number(exp.amount || 0) < 0 ? "text-blue-600" : "text-stone-700"
              }`}
            >
              {Number(exp.amount || 0) < 0 ? "+" : ""}¥
    ...
    <div className="max-w-2xl mx-auto space-y-6 pb-20 animate-in fade-in duration-300">
      ...
      <div className="grid gap-3">
        {menuItems.map((m) => (
          <div
            ...
            className="flex justify-between p-4 bg-white border border-stone-200 rounded-2xl items-center hover:shadow-md transition-all"
          >
            <div>
              <span className="font-bold text-stone-800 block mb-1">
                {m.name || "名称未設定"}
              </span>
              <span className="text-[10px] bg-stone-100 text-stone-500 px-2 py-0.5 rounded font-bold">
                {m.category ||
      ...
      {isCategoryModalOpen && (
        <div className="fixed inset-0 bg-black/50 flex items-center justify-center p-4 z-50">
          <div className="bg-white p-6 rounded-3xl w-full max-w-sm shadow-2xl">
            <h3 className="font-bold mb-2 flex items-center gap-2 text-lg text-stone-800">
            ...
            {categories.length === 0 && (
              <div className="mb-4 bg-orange-50 p-4 rounded-xl border border-orange-200 text-center">
                <Button
            ...
            <div className="space-y-2 mb-6 max-h-60 overflow-y-auto">
              {categories.map((c, i) => (
                <div
                  ...
                  className="flex justify-between items-center p-3 border border-stone-200 rounded-xl bg-stone-50"
                >
            ...
            <form
              ...
              className="flex gap-2 mb-6 border-t pt-4 border-stone-100"
            >
              <input
                type="text"
                ...
                placeholder="新しいカテゴリ"
                className="border border-stone-200 p-3 rounded-xl flex-1 text-sm focus:outline-none focus:ring-2 focus:ring-orange-500 bg-stone-50 focus:bg-white"
              />
            ...
            <Button
              ...
              className="w-full py-3 border-none bg-stone-100"
            >
              閉じる
            </Button>
          </div>
      ...
      {editingMenu && (
        <div className="fixed inset-0 bg-black/50 flex items-center justify-center p-4 z-50 animate-in fade-in duration-200">
          <div className="bg-white p-6 rounded-3xl w-full max-w-md h-[90vh] overflow-y-auto shadow-2xl">
            <h3 className="font-bold mb-6 text-xl text-stone-800">
              メニューの編集
            </h3>
            <form onSubmit={saveMenuItem} className="space-y-5">
              <div>
                ...
                <input
                  ...
                  className="border border-stone-200 bg-stone-50 p-3 w-full rounded-xl focus:bg-white outline-none focus:ring-2 focus:ring-orange-500 font-bold text-stone-700"
                  required
                />
              </div>
              <div>
                ...
                <input
                  name="basePrice"
                  type="number"
                  defaultValue={editingMenu.basePrice || 0}
                  className="border border-stone-200 bg-stone-50 p-3 w-full rounded-xl focus:bg-white outline-none focus:ring-2 focus:ring-orange-500 font-mono text-lg"
                  required
                />
              </div>
              <div>
                ...
                <select
                  ...
                  className="border border-stone-200 bg-stone-50 p-3 w-full rounded-xl focus:bg-white outline-none focus:ring-2 focus:ring-orange-500 text-sm font-bold text-stone-700"
                  required
                >
                  {displayCategories.map((c) => (
                    <option key={c.id || Math.random()} value={c.name}>
                      {c.name}
                    </option>
                  ))}
                </select>
              ...
              <div className="border-t border-stone-100 pt-5">
                <div className="flex justify-between items-center mb-3">
                  ...
                  <button
                    ...
                    className="text-xs bg-stone-100 text-stone-600 px-3 py-1.5 rounded-lg font-bold hover:bg-stone-200"
                  >
                    追加
                  </button>
                </div>
                {editingOptions.map((opt, i) => (
                  <div key={i} className="flex gap-2 mt-2">
                    <input
                      ...
                      className="border border-stone-200 bg-stone-50 p-2 w-full rounded-lg text-sm outline-none focus:ring-1 focus:ring-orange-500"
                      placeholder="名称(大盛など)"
                    />
                    <input
                      type="number"
                      ...
                      className="border border-stone-200 bg-stone-50 p-2 w-24 rounded-lg font-mono text-sm outline-none focus:ring-1 focus:ring-orange-500"
                      placeholder="円"
                    />
                    <button
                      type="button"
              ...
              <div className="border-t border-stone-100 pt-5 space-y-3">
                <label className="flex items-center gap-3 text-sm font-bold text-stone-700 p-2 hover:bg-stone-50 rounded-lg cursor-pointer transition-colors">
                  <input
                    type="checkbox"
                    name="hasSets"
                ...
                <label className="flex items-center gap-3 text-sm font-bold text-green-700 bg-green-50 p-3 rounded-xl cursor-pointer">
                  <input
                    type="checkbox"
                    name="isTakeoutOnly"
              ...
              <div className="flex gap-3 pt-4">
                <Button
                  ...
                  className="flex-1 border-none bg-stone-100"
                >
                  キャンセル
                </Button>
                <Button type="submit" className="flex-1 shadow-orange-200">
                  保存
                </Button>
              </div>
            </form>
    ...
    <div className="max-w-2xl mx-auto space-y-6 pb-20 animate-in fade-in duration-300">
      ...
      {analysisPeriod === "month" && (
        <div className="grid grid-cols-2 gap-4 mb-6">
          <Card className="p-5 border-none shadow-sm bg-gradient-to-br from-blue-50 to-white">
            <h3 className="font-bold text-blue-800 text-xs mb-2 flex items-center gap-1">
              <Users size={14} /> 平均客単価
            </h3>
            <p className="text-2xl font-mono font-bold text-blue-900">
              ¥{advancedAnalytics.avgSpend.toLocaleString()}
            </p>
            <p className="text-[10px] text-blue-600 mt-1">
              来店 {advancedAnalytics.totalCustomers}名
            </p>
          </Card>
          <Card className="p-5 border-none shadow-sm bg-gradient-to-br from-green-50 to-white">
            <h3 className="font-bold text-green-800 text-xs mb-2 flex items-center gap-1">
              <Percent size={14} /> 原価率 (仕入/売上)
            </h3>
            <p className="text-2xl font-mono font-bold text-green-900">
              {advancedAnalytics.costRatio}%
            </p>
            <p className="text-[10px] text-green-600 mt-1">目安は30%前後です</p>
          </Card>
        </div>
      )}
      ...
      <Card className="p-6 border-none shadow-sm">
        <h3 className="font-bold text-stone-700 mb-6 flex items-center gap-2">
          <BarChart3 size={18} className="text-orange-500" /> 売上推移
        </h3>
        <div className="w-full overflow-x-auto pb-2">
          <div
            className={`h-48 flex items-end gap-2 px-2 ${
              analysisPeriod === "month"
                ? "min-w-[600px]"
          ...
          >
            {chartData.map((d, i) => {
              ...
              const height = `${Math.max(
                (Number(d.value || 0) / maxVal) * 100,
                2
              )}%`;
              return (
                <div
                  key={i}
                  className="flex flex-col items-center flex-1 group min-w-[24px]"
                >
                  <div
                    className="w-full bg-gradient-to-t from-orange-200 to-orange-100 rounded-t-lg relative hover:from-orange-400 hover:to-orange-300 transition-all group shadow-sm"
                    style={{ height }}
                  >
                    <div className="absolute -top-8 left-1/2 -translate-x-1/2 text-[10px] bg-stone-800 text-white px-2 py-1 rounded-lg opacity-0 group-hover:opacity-100 whitespace-nowrap z-10 font-mono shadow-lg transition-opacity pointer-events-none">
                      ¥{Number(d.value || 0).toLocaleString()}
                    </div>
                  </div>
                  <span className="text-[9px] text-stone-400 mt-2 whitespace-nowrap font-mono">
                    {d.label || ""}
                  </span>
                </div>
              );
      ...
      {analysisPeriod === "month" &&
        advancedAnalytics.sortedExpenses.length > 0 && (
          <Card className="p-6 border-none shadow-sm">
            <h3 className="font-bold text-stone-700 mb-6 flex items-center gap-2">
              <PieChart size={18} className="text-rose-500" /> 経費の内訳
              (給料除く)
            </h3>
            <div className="h-4 w-full flex rounded-full overflow-hidden mb-4">
              {advancedAnalytics.sortedExpenses.map(([name, amt], i) => {
                const colors = [
                  "bg-rose-500",
                  "bg-pink-400",
                  "bg-purple-400",
                ...
                return (
                  <div
                    ...
                    style={{
                      width: `${
                        (amt / advancedAnalytics.totalExpForChart) * 100
                      }%`,
                    }}
                  ></div>
                );
            ...
            <div className="grid grid-cols-2 gap-3">
              {advancedAnalytics.sortedExpenses.map(([name, amt], i) => {
                ...
                return (
                  <div
                    key={name}
                    className="flex justify-between items-center text-sm border-b border-stone-100 pb-1"
                  >
                    <span
                      className={`font-bold ${
      ...
      {analysisPeriod === "month" && (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <Card className="p-6 border-none shadow-sm">
            ...
            <div className="space-y-3">
              {Object.entries(advancedAnalytics.weatherStats).map(
                ([w, stats]) => {
                  const avg =
                    stats.days > 0 ? Math.floor(stats.sales / stats.days) : 0;
                  return (
                    <div
                      key={w}
                      className="flex justify-between items-center p-2 hover:bg-stone-50 rounded-lg transition-colors"
                    >
                      <span className="text-sm font-bold text-stone-600 w-12">
                        {w}
                      </span>
                      <div className="flex-1 mx-3 h-1.5 bg-stone-100 rounded-full overflow-hidden">
                        <div
                          ...
                          style={{
                            width: `${Math.min((avg / 50000) * 100, 100)}%`,
                          }}
                        ></div>
                      </div>
          ...
          <Card className="p-6 border-none shadow-sm">
            <h3 className="font-bold text-stone-700 mb-4 flex items-center gap-2">
              <Coffee size={18} className="text-orange-500" /> セット注文率
            </h3>
            <div className="flex justify-between items-end mb-2">
              <span className="text-[10px] text-stone-500 font-bold">
                全注文 {advancedAnalytics.totalItems}品 中...
              </span>
              <span className="text-3xl font-mono font-bold text-orange-600">
                {advancedAnalytics.setRate}%
              </span>
            </div>
            <div className="h-6 w-full flex rounded-full overflow-hidden mb-3">
              <div
                className="h-full bg-orange-400 flex items-center justify-center text-white text-[10px] font-bold"
                style={{ width: `${advancedAnalytics.setRate}%` }}
              >
                {advancedAnalytics.setRate > 15 ? "セット" : ""}
              </div>
              <div
                className="h-full bg-stone-200 flex items-center justify-center text-stone-500 text-[10px] font-bold"
                style={{ width: `${100 - advancedAnalytics.setRate}%` }}
              >
                {100 - advancedAnalytics.setRate > 15 ? "単品" : ""}
              </div>
            </div>
            <p className="text-[10px] text-stone-400 text-center">
              セットへの誘導（アップセル）の指標に活用してください
            </p>
          </Card>
        </div>
      )}
      ...
      <Card className="p-6 border-none shadow-sm">
        <h3 className="font-bold text-stone-700 mb-6 flex items-center gap-2">
          <Coffee size={18} className="text-orange-500" /> 人気メニュー TOP5 (
          {currentMonth})
        </h3>
          ...
          {aggregated.menuRanking.map(([name, count], i) => {
            const maxCount =
              aggregated.menuRanking.length > 0
                ? Number(aggregated.menuRanking[0][1] || 1)
                : 1;
            return (
    ...
    <div className="h-full flex flex-col md:flex-row gap-4 overflow-hidden animate-in fade-in duration-300">
      <div className="flex-1 overflow-y-auto pb-20 md:pb-0 pr-2">
        <div className="flex justify-between items-center mb-4">
          <h2 className="text-2xl font-bold text-stone-800 flex items-center gap-2">
            <Utensils className="text-orange-600" /> レジ・注文
          </h2>
        </div>
        ...
        <div className="bg-white p-3.5 rounded-2xl border border-stone-200 mb-5 shadow-sm flex items-center justify-between">
          ...
          <input
            ...
            className="bg-stone-50 border border-stone-200 rounded-xl px-3 py-1.5 text-sm font-mono text-stone-700 font-bold outline-none focus:ring-2 focus:ring-orange-500"
          />
        </div>
        ...
        <div className="grid grid-cols-2 lg:grid-cols-3 gap-3">
          {menuItems
            ...
            .map((item) => (
              <button
                ...
                className={`p-4 rounded-2xl text-left transition-all active:scale-95 shadow-sm border border-stone-100 flex flex-col justify-between h-32 ${
                  item.imageColor || "bg-stone-100"
                } ${
              ...
              >
                {item.isTakeoutOnly && (
                  <span className="absolute top-2 right-2 bg-green-500 text-white text-[10px] font-bold px-2 py-0.5 rounded-full flex items-center gap-1 shadow-sm">
                    <Package size={10} /> To Go
                  </span>
                )}
                <span className="font-bold text-stone-800 leading-tight text-base group-hover:text-orange-900 transition-colors">
                  {item.name || "名称未設定"}
                </span>
                <div className="mt-auto pt-2">
                  <span className="font-mono font-bold text-stone-600 bg-white/60 backdrop-blur-sm px-2.5 py-1 rounded-lg text-sm">
          ...
          <button
            ...
            className="p-4 rounded-2xl flex flex-col justify-center items-center h-32 border-2 border-dashed border-stone-300 text-stone-400 hover:bg-orange-50 hover:border-orange-300 hover:text-orange-500 transition-colors"
          >
            <PlusCircle size={28} className="mb-2" />{" "}
            <span className="text-xs font-bold">メニュー追加</span>
          </button>
        </div>
      </div>
      ...
      <div className="md:w-80 bg-white border-t md:border-l border-stone-200 flex flex-col h-1/3 md:h-full fixed bottom-0 left-0 right-0 md:relative z-10 shadow-2xl md:shadow-none md:rounded-3xl overflow-hidden">
        <div className="p-5 bg-stone-800 text-white flex justify-between items-center shadow-md z-10">
          <span className="font-bold flex items-center gap-2 text-sm uppercase tracking-wider">
            <ShoppingBag size={18} className="text-orange-500" /> Order List
          </span>
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-3 bg-stone-50">
          {cart.length === 0 ? (
            <div className="text-stone-400 text-center flex flex-col items-center justify-center h-full gap-3 opacity-50">
              <ShoppingBag size={48} />
              <p className="font-bold text-sm">注文はまだありません</p>
            </div>
          ) : (
            cart.map((item) => (
              <div
                key={item.tempId || Math.random()}
                className="bg-white p-3.5 rounded-xl shadow-sm flex justify-between items-start border border-stone-100 group"
              >
                <div className="flex-1 pr-2">
                  ...
                  <div className="text-[10px] text-stone-500 flex flex-wrap gap-1.5">
                    {item.setType !== "single" && (
                      <span className="bg-orange-100 text-orange-800 px-1.5 py-0.5 rounded font-bold">
                        {item.setLabel}
                      </span>
                    )}
                    {item.isTakeout ? (
                      <span className="bg-blue-100 text-blue-800 px-1.5 py-0.5 rounded font-bold flex items-center gap-1">
                        <ShoppingBag size={10} /> テイクアウト
                      </span>
                    ) : (
                      <span className="bg-green-100 text-green-800 px-1.5 py-0.5 rounded font-bold flex items-center gap-1">
                        <Home size={10} /> 店内
                      </span>
                    )}
                    {item.options &&
                      ...
                      item.options.map((opt, i) => (
                        <span
                          key={i}
                          className="bg-stone-100 text-stone-600 px-1.5 py-0.5 rounded border border-stone-200"
                        >
                ...
                <div className="flex flex-col items-end gap-2">
                  ...
                  <button
                    ...
                    className="text-stone-300 hover:text-red-500 transition-colors p-1 bg-stone-50 hover:bg-red-50 rounded-lg"
                  >
                    <Trash2 size={14} />
                  </button>
                </div>
        ...
        <div className="p-4 bg-white border-t border-stone-100">
          <Button
            ...
            className="w-full py-4 text-lg shadow-xl shadow-orange-200/50"
      ...
      {selectedItem && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-3xl w-full max-w-md overflow-hidden shadow-2xl animate-in fade-in zoom-in-95 duration-200">
            <div
              className={`p-5 ${
                selectedItem.imageColor || "bg-stone-100"
              } font-bold text-xl flex justify-between items-center text-stone-800 shadow-sm z-10 relative`}
            >
              {selectedItem.name || ""}{" "}
              <button
            ...
            <div className="p-6 space-y-6 max-h-[70vh] overflow-y-auto">
              <div>
                ...
                <div className="space-y-2">
                  <button
                    ...
                    className="w-full text-left p-4 border border-stone-200 rounded-xl hover:bg-stone-50 flex justify-between items-center transition-colors"
                  >
                    <span className="font-bold text-stone-700">単品</span>{" "}
                  ...
                  {(selectedItem.hasSets ||
                    ...
                    <>
                      <button
                        ...
                        className="w-full text-left p-4 border border-orange-200 rounded-xl hover:bg-orange-50 flex justify-between items-center transition-colors"
                      >
                        <div>
                          <span className="block font-bold text-orange-700 text-lg mb-0.5">
                            A Set
                          </span>
                          <span className="text-[10px] font-bold text-stone-500 bg-white px-2 py-0.5 rounded-full border border-stone-100">
                            お好きなドリンク
                          </span>
                        </div>
                  ...
                  {(selectedItem.priceDessertSet ||
                    ...
                    <button
                      ...
                      className="w-full text-left p-4 border border-pink-200 rounded-xl hover:bg-pink-50 flex justify-between items-center transition-colors"
                    >
                      <div>
                        <span className="block font-bold text-pink-700 text-lg mb-0.5">
                          デザートセット
                        </span>
                        <span className="text-[10px] font-bold text-stone-500 bg-white px-2 py-0.5 rounded-full border border-stone-100">
                          お好きなドリンク
                      ...
                      <span className="font-mono text-lg font-bold text-pink-700">
              ...
              {selectedItem.canTakeout !== false && !isTakeoutMode && (
                <div className="pt-6 border-t border-stone-100">
                  <Button
                    variant="secondary"
                    className="w-full py-4 bg-stone-50 hover:bg-blue-50 hover:text-blue-700 hover:border-blue-200"
                    onClick={() => addToCart(selectedItem, "single", true)}
                  >
                    <ShoppingBag size={18} /> 単品でテイクアウトに追加
                  </Button>
                </div>
              )}
            </div>
      ...
      {isCheckoutModalOpen && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-3xl w-full max-w-sm p-8 text-center space-y-8 shadow-2xl animate-in zoom-in-95 duration-200">
            ...
            <div className="py-6 bg-stone-50 rounded-2xl border border-stone-100">
            ...
            <div className="flex gap-3">
              ...
              <Button
                variant="success"
                className="flex-1 py-4 shadow-lg shadow-green-200/50 text-base"
              ...
              <Button
                variant="danger"
                className="flex-1 py-4 shadow-lg shadow-red-200/50 text-base"
                onClick={() => handleCheckout("paypay")}
              >
                PayPay
              </Button>
            </div>
            <p className="text-[10px] text-stone-400 mt-4 leading-relaxed bg-stone-50 p-3 rounded-xl border border-stone-100 text-left">
              ※PayPayを選ぶと、自動的に合計の1.98%が「ランタナ払いの経費」として裏側で記録されます。
            </p>
          </div>
        </div>
      )}
  ...
  if (!user)
    return (
      <div className="h-screen flex flex-col items-center justify-center bg-stone-100 text-stone-400">
        ...
        <span className="font-bold tracking-widest">{loadingStatus}</span>
        {showRetry && (
          <button
            onClick={() => window.location.reload()}
            className="mt-6 text-sm text-blue-500 underline font-bold"
          >
            再読み込み
          </button>
        )}
  ...
  return (
    <div className="h-screen w-full bg-stone-100 text-stone-800 font-sans flex flex-col md:flex-row selection:bg-orange-200 selection:text-orange-900 print:h-auto print:bg-white">
      ...
      <div className="bg-stone-900 text-white p-4 flex md:flex-col justify-between items-center md:w-24 md:h-full z-20 shadow-2xl shrink-0 print:hidden">
        <div className="font-bold text-xl tracking-tighter text-orange-500 md:mb-8 md:mt-2 flex flex-col items-center">
        ...
        <nav className="flex md:flex-col gap-2 md:gap-4 flex-1 justify-center md:justify-start w-full">
          {[
            ...
            <button
              ...
              className={`p-2.5 md:p-3 rounded-2xl flex flex-col items-center gap-1.5 transition-all ${
                activeTab === tab.id
                  ? "bg-orange-500 text-white shadow-lg shadow-orange-900/50 md:scale-110"
        ...
        <button
          ...
          className="md:mt-auto bg-stone-800 p-2.5 rounded-2xl text-[10px] font-bold tracking-widest flex flex-col items-center gap-1 border border-stone-700 hover:bg-stone-700 transition-colors"
        >
      ...
      <main className="flex-1 h-full overflow-hidden relative bg-stone-100 md:rounded-l-3xl md:shadow-[-10px_0_30px_rgba(0,0,0,0.05)] print:overflow-visible print:bg-white print:p-0 print:shadow-none print:w-full print:m-0">
        <header className="h-16 bg-white border-b border-stone-200 flex items-center px-5 justify-between md:hidden shadow-sm z-10 relative print:hidden">
          <span className="font-bold text-stone-800 text-lg tracking-tight">
            ...
            {activeTab === "history" && "売上帳簿・決算"}
            {activeTab === "funds" && "資金管理"}
            {activeTab === "menu" && "メニュー管理"}
            {activeTab === "analysis" && "経営分析"}
          </span>
          <span className="text-xs bg-orange-100 text-orange-800 px-3 py-1.5 rounded-full font-bold shadow-sm">
            担当: {staffName}
          </span>
        </header>
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4 animate-in fade-in zoom-in-95 duration-200 print:hidden">
          <div className="bg-white rounded-3xl w-full max-w-sm p-8 text-center space-y-6 shadow-2xl">
            ...
            <div>
              ...
              <p className="text-sm text-stone-500 mt-3 whitespace-pre-wrap leading-relaxed">
                {deleteModal.message}
              </p>
            </div>
            <div className="flex gap-3 pt-4">
              ...
              <Button
                variant="danger"
                className="flex-1 py-4 shadow-lg shadow-red-200"
